...
public class FfcSfNhsoClaimDataHandler {
    
    private ConnectDatabase.ConnectSQLite sqliteConnection;
    private int updateCount = 0;
    private String tablename = "ffc_sf_nhso_claim_data";
    
    public FfcSfNhsoClaimDataHandler(ConnectDatabase.ConnectSQLite sqliteConnection) {
        this.sqliteConnection = sqliteConnection;
    }
    
    public int updateFfcSfNhsoClaimData(ConnectDatabase.ConnectSQLite sqliteConnection, String lastUpdate) throws SQLException {
        this.sqliteConnection = sqliteConnection;
    ...
    public int checkUpdateCount(ConnectDatabase.ConnectSQLite sqliteConnection, String lastUpdate) throws SQLException {
        ...
        try {
            ResultSet rs = sqliteConnection.getResultSet("SELECT updated_date FROM " + tablename);
            while (rs.next()) {
                if (rs.getString("updated_date") != null) {
                    Timestamp ts2 = Timestamp.valueOf(rs.getString("updated_date"));
    ...
    public boolean checkAndCreateTable() throws SQLException {
        try {
            ...
            Logger.getLogger(FfcSfNhsoClaimDataHandler.class.getName()).log(Level.SEVERE, null, ex);
            return false;
        }
    }
    
    private boolean createTable() throws SQLException {
        String createTableSQL = "CREATE TABLE `ffc_sf_nhso_claim_data` ("
