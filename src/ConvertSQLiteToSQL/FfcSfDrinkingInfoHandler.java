...
public class FfcSfDrinkingInfoHandler {
    
    private ConnectDatabase.ConnectSQLite sqliteConnection;
    private int updateCount = 0;
    
    public FfcSfDrinkingInfoHandler(ConnectDatabase.ConnectSQLite sqliteConnection) {
        this.sqliteConnection = sqliteConnection;
    }
    
    public int updateFfcSfDrinkingInfo(ConnectDatabase.ConnectSQLite sqliteConnection, String lastUpdate) throws SQLException {
        this.sqliteConnection = sqliteConnection;
        if (!checkAndCreateTable()) {
            System.out.println("Cannot proceed with ffc_sf_drinking_info update - table creation failed.");
            return 0;
        }
        return this.updateCheckDateupdate(sqliteConnection,"ffc_sf_drinking_info", lastUpdate);
    }
    ...
    public int checkUpdateCount(ConnectDatabase.ConnectSQLite sqliteConnection, String lastUpdate) throws SQLException {
        ...
        try {
            ResultSet rs = sqliteConnection.getResultSet("SELECT dateupdate FROM ffc_sf_drinking_info");
            while (rs.next()) {
            ...
            System.out.println("Table ffc_sf_drinking_info not found in SQLite database: " + ex.getMessage());
            return 0;
        }
        return updateCount;
    }
    ...
    public boolean updateForNewVisit(ConnectDatabase.ConnectSQLite sqliteConnection, String visitInsert, int visitMaxNew) throws SQLException {
        ...
        return this.insertOtherVisit(sqliteConnection, "ffc_sf_drinking_info", visitMaxNew, visitInsert);
    }
        ...
        try {
            ...
            if (rs.next()) {
                ...
                while (rss.next()) {
                    ...
                    updateCount++;
                    System.out.println("Insert ffc_sf_drinking_info: " + insertData1 + insertData2);
    ...
    private int updateCheckDateupdate(ConnectDatabase.ConnectSQLite sqliteConnection, String tablename, String lastUpdate) throws SQLException {
        ...
        while (rs.next()) {
            ...
            if (rss.next()) {
                System.out.println("Update ffc_sf_drinking_info data");
                this.updateDataOneRow(tablename, arrayRs);
            } else {
                System.out.println("Insert ffc_sf_drinking_info data");
    ...
    private void insertData(String tableName, ArrayList<ResultSet> dataInsert) throws SQLException {
        ...
        for (ResultSet data : dataInsert) {
            ...
            System.out.println("Insert ffc_sf_drinking_info: " + insertQuery1 + insertQuery2);
    ...
    private void updateDataOneRow(String tableName, ArrayList<ResultSet> data) throws SQLException {
        ...
        for (ResultSet dataUpdate : data) {
            ...
            System.out.println("Update ffc_sf_drinking_info: " + updateQuery);
    ...
    public boolean checkAndCreateTable() throws SQLException {
        try {
            String checkTableQuery = "SELECT COUNT(*) AS table_count FROM information_schema.tables "
                    + "WHERE table_schema = DATABASE() AND table_name = 'ffc_sf_drinking_info'";
            ...
            if (tableCount == 0) {
                System.out.println("Table ffc_sf_drinking_info not found. Creating table...");
                return createTable();
            }
            return true;
        } catch (SQLException ex) {
            Logger.getLogger(FfcSfDrinkingInfoHandler.class.getName()).log(Level.SEVERE, null, ex);
            return false;
        }
    }
