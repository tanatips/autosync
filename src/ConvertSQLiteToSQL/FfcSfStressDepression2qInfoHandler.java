...
public class FfcSfStressDepression2qInfoHandler {
    
    private ConnectDatabase.ConnectSQLite sqliteConnection;
    private int updateCount = 0;
    private String tablename = "ffc_sf_stress_depression_2q_info";
    
    public FfcSfStressDepression2qInfoHandler(ConnectDatabase.ConnectSQLite sqliteConnection) {
        this.sqliteConnection = sqliteConnection;
    }
    
    public int updateFfcSfStressDepression2qInfo(ConnectDatabase.ConnectSQLite sqliteConnection, String lastUpdate) throws SQLException {
        this.sqliteConnection = sqliteConnection;
        if (!checkAndCreateTable()) {
            System.out.println("Cannot proceed with "+tablename+" update - table creation failed.");
            return 0;
        }
        return this.updateCheckDateupdate(sqliteConnection, tablename, lastUpdate);
    }
    ...
    public boolean checkAndCreateTable() throws SQLException {
        try {
            ...
            Logger.getLogger(FfcSfStressDepression2qInfoHandler.class.getName()).log(Level.SEVERE, null, ex);
            return false;
        }
    }
    
    private boolean createTable() throws SQLException {
        String createTableSQL = "CREATE TABLE `ffc_sf_stress_depression_2q_info` ("
