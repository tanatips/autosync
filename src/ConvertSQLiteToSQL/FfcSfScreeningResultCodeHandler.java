...
public class FfcSfScreeningResultCodeHandler {
    
    private ConnectDatabase.ConnectSQLite sqliteConnection;
    private int updateCount = 0;
    private String tablename = "ffc_sf_screening_result_code";
    
    public FfcSfScreeningResultCodeHandler(ConnectDatabase.ConnectSQLite sqliteConnection) {
        this.sqliteConnection = sqliteConnection;
    }
    
    public int updateFfcSfScreeningResultCode(ConnectDatabase.ConnectSQLite sqliteConnection, String lastUpdate) throws SQLException {
        this.sqliteConnection = sqliteConnection;
    ...
    public int checkUpdateCount(ConnectDatabase.ConnectSQLite sqliteConnection, String lastUpdate) throws SQLException {
        ...
        try {
            ResultSet rs = sqliteConnection.getResultSet("SELECT updatetime FROM " + tablename);
            while (rs.next()) {
                if (rs.getString("updatetime") != null) {
                    Timestamp ts2 = Timestamp.valueOf(rs.getString("updatetime"));
    ...
    private int updateCheckDateupdate(ConnectDatabase.ConnectSQLite sqliteConnection, String tablename, String lastUpdate) throws SQLException {
        int count = 0;
        String sqliteQuery = "SELECT * FROM " + tablename + " WHERE updatetime > '" + lastUpdate + "'";
    ...
    private String getQueryWhereCondition(String tableName, ResultSet rs) throws SQLException {
        if (this.tablename.equals(tableName)) {
            return " WHERE person_id ='" + rs.getString("person_id") 
                   + "' AND visitno = '" + rs.getString("visitno") 
                   + "' AND screening_type = '" + rs.getString("screening_type") + "'";
        }
        return "";
    }
    ...
    public boolean checkAndCreateTable() throws SQLException {
        try {
            ...
            Logger.getLogger(FfcSfScreeningResultCodeHandler.class.getName()).log(Level.SEVERE, null, ex);
            return false;
        }
    }
    
    private boolean createTable() throws SQLException {
        String createTableSQL = "CREATE TABLE `ffc_sf_screening_result_code` ("
