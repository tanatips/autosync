...
public class FfcSfSmokerInfoHandler {
    
    private ConnectDatabase.ConnectSQLite sqliteConnection;
    private int updateCount = 0;
    
    public FfcSfSmokerInfoHandler(ConnectDatabase.ConnectSQLite sqliteConnection) {
        this.sqliteConnection = sqliteConnection;
    }
    
    public int updateFfcSfSmokerInfo(ConnectDatabase.ConnectSQLite sqliteConnection, String lastUpdate) throws SQLException {
        this.sqliteConnection = sqliteConnection;
        if (!checkAndCreateTable()) {
            System.out.println("Cannot proceed with ffc_sf_smoker_info update - table creation failed.");
            return 0;
        }
        return this.updateCheckDateupdate(sqliteConnection, "ffc_sf_smoker_info", lastUpdate);
    }
    
    public int checkUpdateCount(ConnectDatabase.ConnectSQLite sqliteConnection, String lastUpdate) throws SQLException {
        if (!checkAndCreateTable()) {
            System.out.println("Cannot check ffc_sf_smoker_info updates - table creation failed.");
            return 0;
        }
        ...
        try {
            ResultSet rs = sqliteConnection.getResultSet("SELECT dateupdate FROM ffc_sf_smoker_info");
            while (rs.next()) {
            ...
            System.out.println("Table ffc_sf_smoker_info not found in SQLite database: " + ex.getMessage());
            return 0;
        }

        System.out.println("ffc_sf_smoker_info Update Count : " + updateCount);
        return updateCount;
    }
    ...
    public boolean updateForNewVisit(ConnectDatabase.ConnectSQLite sqliteConnection, String visitInsert, int visitMaxNew) throws SQLException {
        if (!checkAndCreateTable()) {
            System.out.println("Cannot update ffc_sf_smoker_info for visit - table creation failed.");
            return false;
        }
        return this.insertOtherVisit(sqliteConnection, "ffc_sf_smoker_info", visitMaxNew, visitInsert);
    }
        ...
        try {
            ...
            if (rs.next()) {
                ...
                while (rss.next()) {
                    ...
                    updateCount++;
                    System.out.println("Insert ffc_sf_smoker_info: " + insertData1 + insertData2);
    ...
    private int updateCheckDateupdate(ConnectDatabase.ConnectSQLite sqliteConnection, String tablename, String lastUpdate) throws SQLException {
        ...
        while (rs.next()) {
            ...
            if (rss.next()) {
                System.out.println("Update ffc_sf_smoker_info data");
                this.updateDataOneRow(tablename, arrayRs);
            } else {
                System.out.println("Insert ffc_sf_smoker_info data");
    ...
    private String getQueryWhereCondition(String tableName, ResultSet rs) throws SQLException {
        String value = "";
        if ("ffc_sf_smoker_info".equals(tableName)) {
    ...
    private void insertData(String tableName, ArrayList<ResultSet> dataInsert) throws SQLException {
        ...
        for (ResultSet data : dataInsert) {
            ...
            System.out.println("Insert ffc_sf_smoker_info: " + insertQuery1 + insertQuery2);
    ...
    private void updateDataOneRow(String tableName, ArrayList<ResultSet> data) throws SQLException {
        ...
        for (ResultSet dataUpdate : data) {
            ...
            System.out.println("Update ffc_sf_smoker_info: " + updateQuery);
    ...
    public boolean checkAndCreateTable() throws SQLException {
        try {
            String checkTableQuery = "SELECT COUNT(*) AS table_count FROM information_schema.tables "
                    + "WHERE table_schema = DATABASE() AND table_name = 'ffc_sf_smoker_info'";
            ...
            if (tableCount == 0) {
                System.out.println("Table ffc_sf_smoker_info not found. Creating table...");
                if (createTable()) {
                    System.out.println("Table ffc_sf_smoker_info created successfully.");
                    return true;
                } else {
                    System.out.println("Failed to create table ffc_sf_smoker_info.");
                    return false;
                }
            } else {
                System.out.println("Table ffc_sf_smoker_info already exists.");
                return true;
            }
        } catch (SQLException ex) {
            Logger.getLogger(FfcSfSmokerInfoHandler.class.getName()).log(Level.SEVERE, null, ex);
            System.out.println("Error checking/creating table ffc_sf_smoker_info: " + ex.getMessage());
            return false;
        }
    }
    
    private boolean createTable() throws SQLException {
        String createTableSQL = "CREATE TABLE `ffc_sf_smoker_info` ("
        ...
        try {
            ...
            Logger.getLogger(FfcSfSmokerInfoHandler.class.getName()).log(Level.SEVERE, null, ex);
            System.out.println("Error creating table ffc_sf_smoker_info: " + ex.getMessage());
            return false;
        }
    }
